...
import com.app.preorder.common.exception.custom.DuplicateValueException;
import com.app.preorder.common.exception.custom.EmailResendLimitException;
...
public class MemberServiceImpl implements MemberService {
    ...
    public void signup(SignupRequest request) {
        ...
        if (autoVerify) {
            // ✅ Feign 호출 → Kafka Outbox 발행으로 교체
            memberCommandPublisher.publishCartCreateCommand(
                    member.getId(),
                    member.getLoginId(),
                    member.getEmail()
            );
            log.info("[MemberService] auto-verify 회원 가입 - 카트 생성 이벤트 발행, memberId={}", member.getId());
        } else {
    ...
    public void confirmEmailVerification(String key) {
        ...
        // ✅ Feign 제거 → Kafka Outbox 발행으로 교체
        memberCommandPublisher.publishCartCreateCommand(
                member.getId(),
                member.getLoginId(),
                member.getEmail()
        );
        log.info("[MemberService] 이메일 인증 완료 - 카트 생성 이벤트 발행, memberId={}", member.getId());
